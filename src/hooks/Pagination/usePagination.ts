...
const usePagination = (
  handlePaginationPage: (page: number) => Promise<unknown[]>,
  handleLimitUser: (limit: number) => Promise<unknown[]>,
  ...
  const [isActivePreviousIcon, setIsActivePreviousIcon] =
  ...
  useEffect(() => {
    const checkRecordPerPage = limit
      ? users.length < limit
      : users.length < LIMIT_DEFAULT;
  ...
  return {
    ...
    hasPagination:
      pageRef.current !== PAGE_DEFAULT ||
      (pageRef.current === PAGE_DEFAULT && users.length > 0),
  };
};
